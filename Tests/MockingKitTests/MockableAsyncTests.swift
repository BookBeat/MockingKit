...
class MockableAsyncTests: XCTestCase {
    ...
    func testStreamingCallsReportsCallsWhenTheyAreReportedOverTime() async {
        mock.registerResult(for: \.functionWithIntResultRef) { _, arg2 in arg2 }
        ...
        var calls = [Int]()
        for await call in mock.streamedCalls(to: \.functionWithIntResultRef, terminationStrategy: .defaultTimeout) {
    ...
    func testStreamingCallsWithRegisteredCallsTermination() async {
        ...
        var calls = [Int]()
        for await call in mock.streamedCalls(to: \.functionWithIntResultRef, terminationStrategy: .registeredCalls(2)) {
            calls.append(call.arguments.1)
        }
